...
var DateTime = function(input, opts) {
  ...
  var structure = $(
    '<div class="' + classPrefix + '"><div class="' + classPrefix + '-date"><div class="' + classPrefix + '-title"><div class="' + classPrefix + '-iconLeft"><button type="button"></button></div><div class="' + classPrefix + '-iconRight"><button type="button"></button></div><div class="' + classPrefix + '-label"><span></span><select class="' + classPrefix + '-month"></select></div><div class="' + classPrefix + '-label"><span></span><select class="' + classPrefix + '-year"></select></div></div><div class="' + classPrefix + '-buttons"><a class="' + classPrefix + '-clear"></a><a class="' + classPrefix + '-today"></a></div><div class="' + classPrefix + '-calendar"></div></div><div class="' + classPrefix + '-time"><div class="' + classPrefix + '-hours"></div><div class="' + classPrefix + '-minutes"></div><div class="' + classPrefix + '-seconds"></div></div><div class="' + classPrefix + '-error"></div></div>'
  );
  this.dom = {
    container: structure,
...
$.extend(DateTime.prototype, {
  ...
  val: function(set, write) {
    ...
    this.s.display = this.s.d ? new Date(this.s.d.toString()) : /* @__PURE__ */ new Date();
    ...
    this.dom.container.on("change", "select", function() {
      ...
      if (nodeName === "a") {
        ...
        if ($(target).hasClass(classPrefix + "-clear")) {
          ...
          that.s.display = /* @__PURE__ */ new Date();
          that._setTitle();
          that._setCalander();
        }
      }
  ...
  _compareDates: function(a, b) {
    return this._isLuxon() ? dateLib.DateTime.fromJSDate(a).toUTC().toISODate() === dateLib.DateTime.fromJSDate(b).toUTC().toISODate() : this._dateToUtcString(a) === this._dateToUtcString(b);
  },
  /**
  ...
  _convert: function(val, from, to) {
    ...
    if (!dateLib) {
      if (!from && !to || from && to) {
        ...
        return val.getUTCFullYear() + "-" + this._pad(val.getUTCMonth() + 1) + "-" + this._pad(val.getUTCDate());
      } else {
        var match = val.match(/(\d{4})\-(\d{2})\-(\d{2})/);
        return match ? new Date(match[1], match[2] - 1, match[3]) : null;
      }
    } else if (this._isLuxon()) {
      var dtLux = val instanceof Date ? dateLib.DateTime.fromJSDate(val).toUTC() : dateLib.DateTime.fromFormat(val, from);
      if (!dtLux.isValid) {
        return null;
      }
      return to ? dtLux.toFormat(to) : dtLux.toJSDate();
    } else {
      var dtMo = val instanceof Date ? dateLib.utc(val, void 0, this.c.locale, this.c.strict) : dateLib(val, from, this.c.locale, this.c.strict);
      if (!dtMo.isValid()) {
        return null;
      }
  ...
  _dateToUtcString: function(d) {
    return this._isLuxon() ? dateLib.DateTime.fromJSDate(d).toUTC().toISODate() : d.getUTCFullYear() + "-" + this._pad(d.getUTCMonth() + 1) + "-" + this._pad(d.getUTCDate());
  },
  /**
  ...
  _htmlMonth: function(year, month) {
    var now = this._dateToUtc(/* @__PURE__ */ new Date()), days = this._daysInMonth(year, month), before = new Date(Date.UTC(year, month, 1)).getUTCDay(), data = [], row = [];
  ...
  _htmlWeekOfYear: function(d, m, y) {
    ...
    var weekNum = Math.ceil(((date - oneJan) / 864e5 + 1) / 7);
   ...
   */
  _isLuxon: function() {
    return dateLib && dateLib.DateTime && dateLib.Duration && dateLib.Settings ? true : false;
  },
  /**
  ...
  _optionsTime: function(unit, count, val, allowed, range) {
    ...
    var i, j;
    var render = count === 12 ? function(i2) {
      return i2;
    } : this._pad;
    var classPrefix = this.c.classPrefix;
    var className = classPrefix + "-table";
    ...
    var a = "";
    var span = 10;
    var button = function(value, label, className2) {
    ...
    if (count === 12) {
      ...
      var start = range !== null ? range : val === -1 ? 0 : Math.floor(val / 10) * 10;
      a += "<tr>";
    ...
    container.empty().append(
      '<table class="' + className + '"><thead><tr><th colspan="' + span + '">' + i18n[unit] + "</th></tr></thead><tbody>" + a + "</tbody></table>"
    );
  },
  /**
  ...
  _optionsTitle: function() {
    ...
    var i = minYear !== null ? minYear : (/* @__PURE__ */ new Date()).getFullYear() - this.c.yearRange;
    var j = maxYear !== null ? maxYear : (/* @__PURE__ */ new Date()).getFullYear() + this.c.yearRange;
    this._options("month", this._range(0, 11), i18n.months);
    this._options("year", this._range(i, j));
    this.dom.today.text(i18n.today).text(i18n.today);
    this.dom.clear.text(i18n.clear).text(i18n.clear);
    this.dom.previous.attr("title", i18n.previous).children("button").text(i18n.previous);
  ...
  _setCalander: function() {
    if (this.s.display) {
      this.dom.calendar.empty().append(this._htmlMonth(
...
var dataTables_dateTime_default = DateTime;
export {
  dataTables_dateTime_default as default
};
...
datatables.net-datetime/dist/dataTables.dateTime.mjs:
   ...
   *)
*/
//# sourceMappingURL=datatables__net-datetime.js.map
